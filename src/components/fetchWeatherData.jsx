...
export const fetchWeatherData = async ({ latitude, longitude, cityName, isCelsius }) => {
  try {
    ...
    if (latitude && longitude) {
      weatherUrl = `https://api.openweathermap.org/data/2.5/weather?lat=${latitude}&lon=${longitude}&appid=${API_KEY}&units=${isCelsius ? "metric" : "imperial"}&lang=de`;
      forecastUrl = `https://api.openweathermap.org/data/2.5/forecast?lat=${latitude}&lon=${longitude}&appid=${API_KEY}&units=${isCelsius ? "metric" : "imperial"}&lang=de`;
    } else {
      weatherUrl = `https://api.openweathermap.org/data/2.5/weather?q=${cityName}&appid=${API_KEY}&units=${isCelsius ? "metric" : "imperial"}&lang=de`;
      forecastUrl = `https://api.openweathermap.org/data/2.5/forecast?q=${cityName}&appid=${API_KEY}&units=${isCelsius ? "metric" : "imperial"}&lang=de`;
    }
